...
class CuponRepository:
    ...
    def obtener_por_codigo(codigo: str) -> CuponModel:
        return CuponModel.objects.get(codigo=codigo)
    ...
    def es_disponible(codigo: str) -> bool:
        ...
        return (
            cupon.fecha_inicio.date() <= hoy <= cupon.fecha_fin.date()
            and cupon.veces_usado < cupon.usos
        )
