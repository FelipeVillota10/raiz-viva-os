...
export default function AprobacionesPage() {
  ...
  const { solicitudes, pendingCount } = useAprobaciones();
  const [filtro, setFiltro] = useState<string>('');

  const filteredSolicitudes = filtro
    ? solicitudes.filter((s: Solicitud) => s.estado_resultado === filtro)
    : solicitudes;
    ...
    <div className="px-4 py-8 max-w-4xl mx-auto w-full">
      ...
      {filteredSolicitudes.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredSolicitudes.map((solicitud: Solicitud) => {
