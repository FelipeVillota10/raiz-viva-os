...
const CAMPOS: { key: keyof Itinerario; label: string; placeholder: string }[] = [
  ...
  { key: "notas_especiales", label: "Notas especiales", placeholder: "Información adicional relevante..." },
];
...
export function FormItinerario({ ecoaventuraId, nombreEco, onCerrar }: Props) {
  ...
  useEffect(() => {
    getItinerario(ecoaventuraId).then((it) => {
      if (it) setForm(it);
    }).finally(() => setCargando(false));
  }, [ecoaventuraId]);

  const set = (campo: keyof Itinerario, valor: string) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("Error al guardar el itinerario. Intenta de nuevo.");
    } finally {
      setGuardando(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black/40 flex items-start justify-center z-50 overflow-y-auto py-8 px-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl">
        <div className="flex items-center justify-between p-5 border-b">
          <div>
            <h3 className="font-semibold text-[#3a5c2e] text-lg">Itinerario & Logística</h3>
        ...
        {cargando ? (
          <div className="p-8 text-center text-gray-400 text-sm animate-pulse">Cargando itinerario...</div>
        ) : (
