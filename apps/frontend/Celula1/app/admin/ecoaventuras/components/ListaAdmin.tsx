...
export function ListaAdmin({ ecoaventuras, onEditar, onItinerario, onActualizar }: Props) {
  const handleToggle = async (eco: EcoAventura) => {
    try {
      onActualizar(await toggleActivo(eco.id));
    } catch {
  ...
  return (
    <div className="overflow-x-auto rounded-2xl shadow-sm">
      <table className="w-full text-sm bg-white">
        <thead>
          <tr className="bg-[#3b5630] text-white text-left">
          ...
          {ecoaventuras.map((eco) => (
            <tr key={eco.id} className={`hover:bg-gray-50 transition ${!eco.activo ? "opacity-50" : ""}`}>
              ...
              <td className="px-4 py-3 text-center">
                <button onClick={() => handleToggle(eco)}
              ...
              <td className="px-4 py-3">
                <div className="flex gap-2 justify-center">
                  <button onClick={() => onEditar(eco)}
                    className="text-xs bg-[#557149] hover:bg-[#3b5630] text-white px-3 py-1 rounded-lg transition">
                    Editar
                  </button>
                  <button onClick={() => onItinerario(eco)}
